    ...
    return (
        <main className='flex-grow'>
            ...
            <section className='sm:min-h-[80vh] grid place-items-center max-w-7xl mx-auto w-full'>
                <div className={`flex flex-col gap-10 xl:gap-16 lg:flex-row items-center justify-center w-full mx-auto my-10 p-5 py-10 sm:px-10 xl:px-16 min-h-[70vh]`}>
                    <div className='flex-grow'>
                        <h1 className='font-semibold text-rose-500 text-4xl sm:text-5xl my-3 text-center lg:text-left lg:max-w-md'>Limitations</h1>
                        <p className='text-gray-500 max-w-2xl lg:max-w-sm 2xl:max-w-xl 2xl:text-lg text-center mx-auto lg:mx-0 lg:text-left leading-relaxed'>In order to sync your heart in reality with the app, a hardware device is required to detect that just like smart watches. This allows the user to carry it with them every time.</p>
                        <p className='text-gray-500 max-w-2xl lg:max-w-sm 2xl:max-w-xl 2xl:text-lg text-center mx-auto lg:mx-0 lg:text-left leading-relaxed'>Additionally, it can be inconvenient and uncomfortable to use and affect the volition in many ways. Therefore, it is often not a viable option for many users.</p>
                    </div>
                    <div className='max-w-lg w-full h-[400px] sm:h-[500px] rounded-2xl overflow-hidden flex-grow relative'>
                        <Image placeholder="blur" blurDataURL={Three.blurDataURL} src={Three.src} priority={false} loading="lazy" className='object-cover hover:scale-105 transition-transform duration-300 ease-in-out drop-shadow' alt="trial features" fill={true} />
                    </div>
            ...
            <section className='min-h-screen w-full p-5 flex flex-col items-center justify-center'>
                ...
                <div className={`h-[500px] w-full max-w-7xl rounded-xl overflow-hidden relative mt-10 mb-32`}>
                    <Image blurDataURL={Team.blurDataURL} src={Team.src}  placeholder="blur" priority={false} loading="lazy" height={500} width={500} className='w-full h-full object-cover absolute inset-0' alt="" />
